...
fix_Ramo <- function(data_frame, column) {
  for (i in 1:nrow(data_frame)) {
    ifelse(is.na(data_frame[i ,column]),
           data_frame[i ,column] <- 'no',
           data_frame[i ,column] <- paste('ramo', data_frame[i ,column]))
  }
  data_frame
}


fix_Folio <- function(data_frame, column) {
  for (i in 1:nrow(data_frame)) {
    ifelse(is.na(data_frame[i ,column]),
           data_frame[i ,column] <- 0,
           data_frame[i ,column] <- data_frame[i ,column])
  }
  data_frame
}
